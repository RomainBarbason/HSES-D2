...
def perform_cpa_attack(trace_file_path, cleartext_data, byte_index):
    ...
    Returns:
        The most likely key byte and correlation values for all key guesses
    """
    ...
    for key_guess in range(256):
        print(f"Attempting byte {key_guess} for byte {byte_index}...", end="")
        ...
        print(f"[debug]Max correlation value: {max_correlations[key_guess]}")
    
    # Find the key byte with highest correlation
    best_key_byte = np.argmax(max_correlations)
    print(f"[debug]--- Best candidate for byte {byte_index}: {best_key_byte} ---")
    
    return best_key_byte, max_correlations

def attack_all_bytes(dataset_num=1):
    """
    Attack all 16 bytes of the AES key
    ...
    # Perform the attack for each byte
    recovered_key = []
    for byte_index in range(16):
        trace_file = f"{dataset_prefix}{byte_index}.txt"
        key_byte, correlations = perform_cpa_attack(trace_file, cleartext_data, byte_index)
        recovered_key.append(key_byte)
        
        # Print results for this byte
        print(f"Byte {byte_index}: 0x{key_byte:02x} (correlation: {np.max(correlations):.4f})")
    ...
    print(f"\nRecovered Key: {[hex(b) for b in recovered_key]}")
...
def main():
    ...
    # Example for attacking just one byte (byte 0) from dataset 1
    # You would need to adjust the file paths based on your setup
    cleartext_data = load_cleartext(cleartext_file)
    trace_file = "dataset1/trace0.txt"  # For byte 0

    print("Attacking byte 0...")
    key_byte, correlations = perform_cpa_attack(trace_file, cleartext_data, 0)
    print(f"Most likely key for byte 0: 0x{key_byte:02x}")
    ...
    plt.xlabel('Key Byte Guess')
    plt.ylabel('Maximum Correlation')
    plt.title('Correlation for Byte 0')
    plt.show()
    
    # Uncomment to run the full attack on all bytes
    # print("\nPerforming full key recovery for dataset 1...")
